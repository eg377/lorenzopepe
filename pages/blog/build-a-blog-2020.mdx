...
import { IrradiationIllusion} from '../../Components/BlogPostsSnippets/IrradiationIllusion';
...
frameworks, the reason I chose Next.js is because it's API is a very thin layer on top of React with 2 or 3 functions added on top depending on your use case. Not only it's that simple, 
...
sponsored nor paid for by the Next.js guys.
...
2. ### Building the useDarkMode hook
	...
	```typescript numbered [useDarkMode.ts] {12-21, 26-32}
	import { useState } from "react";
	...
	And it can be easily hooked up to a component such as this:
...
1. Typically, inside a Next.js blog with simple Markdown there exist a way of using dynamic routes with `getStaticPaths` and `getStaticProps` to fetch
	...
	reading a [blog post](https://blog.tailwindcss.com/building-the-tailwind-blog) by the tailwind.css guys and they did face similar problems.
...
The usage is straightforward. First of all you need to grab a theme.If you are using VSCode you can use `CTRL + Shift + P` to open the Command Palette and choose the 
